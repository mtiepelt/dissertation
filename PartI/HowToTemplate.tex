% !TEX encoding = UTF-8 Unicode
% !TEX root = ../diss.tex



% -----------------------------------------------------
\cleardoublepage
\chapter{How to use this Template}

\section{Dummy Section}

We need a section to show that in the ToC the numbers of the ``sections'' also contain a link to the respective page.


\lipsum[1-1]%
\footnote{\detokenize{\footnote}: This is a numbered and referenced footnote in the margin.} 
\lipsum[2-2]%
\sidenote{\detokenize{\sidenote}: This is a numbered but \textbf{un}referenced footnote in the margin.}
\sidecomment{\detokenize{\sidecomment}: This is a unnumbered and unreferenced comment in the margin.}


\paragraph{Citattion}

Citations \cite{cryptoeprint:2021/1484} appear as preview in the margin. 
Floats cannot have footnotes (without modifications) and citations with \detokenize{\cite} act like footnotes in this template. 
Consequently, 
you need to use \detokenize{\citeonly} instead, 
which \emph{only} print the citation in the text but not in the margin. 
Additionally, 
if there is too little space in the margin, 
LaTeX might fail to find a valid positioning. 
This might, for example, cause \emph{latexmk} too loop infinitely (or throw an error). 
To easiest to avoid this is to limit the number of note in the margin, especially if there is a \emph{full-width figure} on the page.


Floats include (but are not limited to):
\begin{itemize}
	\item captions
	\item footnote, sidenote, sidecomment
\end{itemize}

The class offers a \emph{full} citation:
\begin{itemize}
 	\item \detokenize{\fullfullcite}
 \end{itemize} 

\fullfullcite{cryptoeprint:2020/367}

There is also a command to present your contributions:

\begin{itemize}
	\item \detokenize{\contentsource}[TEXT]{OFFICIAL CITETATION}{OPEN CITETATION}{Contribution}[IMPLEMENTATION]
\end{itemize}




% -----------------------------------------------------
\paragraph{Print vs Digital Version}

We consider the print version to be two-sided, and the digital version to be one-sided. This can be changed as option to the documentclass.

\begin{itemize}
	\item \detokenize{\documentclass[oneside]{dissertation}}
	\item \detokenize{\documentclass[twoside]{dissertation}}
\end{itemize}

Both versions should have the same page numbers. This sometimes causes the one-sided version to have empty pages. If you want only one-sided version, search for ``Match page numbers of one-sided and two-sided document'' in the ``dissertation.sty'' package.

Additionally, the class file ensures that references occur only once on each visible page, that means once on every one-sided pages, and once on every double page for the two-sided version.

In ``\url{0_config/macros.tex}'' we provide two macros that change the behavior of LaTeX according to the option.

\begin{itemize}
 	\item \detokenize{\linktocode{LINK}{WORD}}, for example, \linktocode{https://github.com/mtiepelt/dissertation}{implementation}
 	\begin{itemize}
 		\item embeds the link into a reference for one-sided
 		\item prints a footnote with the reference for two-sided
 	\end{itemize}
 	%
 	\item The second macro changes the behavior of \detokenize{\cleardoublepage} such that one-sided and two-sided documents have the same page numbers and numbering. That that there should be a \detokenize{\cleardoublepage} every time LaTeX makes a new page, for example, in front of \detokenize{\part}, \detokenize{\include}, \detokenize{\chapter}, ... 
 \end{itemize} 



% -----------------------------------------------------
\paragraph{Fullwidth}

The classfile offers to use the full width of the page:

\begin{itemize}
	\item The environment ``fullwidth'' sets the fullwidth for a page. 
	\item The environment ``fullwidthfig'' can be used to only have part of a page (for example a figure) span the full width. Using ``fullwidthfig'' outside of a float can result issues with the margins. 
\end{itemize}


\paragraph{Floats}

You might want to put \detokenize{\blockmargin}, \detokenize{\unblockmargin} around your ``fullwidth'' floats to avoid LaTeX trying to put a float in the (non-existing) margin next to it. 

\blockmargin
\begin{figure}[th]
\begin{fullwidthfig}
	\contentsource[TEXT.]{DBLP:conf/ccs/TiepeltD20}{cryptoeprint:2020/367}{Everyone did everything.}[\url{Link to Github}]
\end{fullwidthfig}
\end{figure}
\unblockmargin

\clearpage
\paragraph{Figures and Captions}

\begin{figure}[h]
	\includegraphics[width=\linewidth]{example-image-a}
	%
	\caption{Linewidth Figure}
	\label{LABEL1}
\end{figure}

\blockmargin
\begin{figure}[h]
	\begin{fullwidthfig}
	\includegraphics[width=\linewidth]{example-image-a}
	%
	\caption{Fullwidth Figure}
	\label{LABEL2}
	\end{fullwidthfig}
\end{figure}
\unblockmargin


\begin{figure}[h]
	\begin{sidecaption}{Sidecaption}
		%
		\includegraphics[width=\linewidth]{example-image-a}
		%
		\label{LABEL3}
	\end{sidecaption}
\end{figure}

\begin{figure}[h]
	%
	\centering 
	\subbottom[{Subfigure in Memoir class}\label{LABEL4}]{
          \includegraphics[width=.7\linewidth]{example-image-a}
    }\\%
    \subbottom[{Subfigure in Memoir class}\label{LABEL5}]{
          \includegraphics[width=.7\linewidth]{example-image-b}
    }%
    \caption{Caption of subfigure in Memoir.}
    \label{LABEL6}
\end{figure}


\clearpage
% -----------------------------------------------------
\paragraph{Acronyms}

This is a acronym: \gls{AES}. In \lstinline{disseratationpackage.sty} there are a few lines that allow to link only the first occurence. Search for ``LINK'', and if you uncomment those lines, then 
\begin{itemize}
	\item This should not have a hyperref: \gls{AES}. 
	\item You can reset the reference using \detokenize{\glsresetall}, for example, after every new part. 
\glsresetall{} 
Now it should be referenced and expanded again: \gls{AES} 
\end{itemize}

% -----------------------------------------------------
\paragraph{Other Things}

\begin{itemize}
 	\item If you use a \detokenize{\chapter*{CHAPTER}} you might want to add a line \detokenize{\sectionmark{CHAPTER}} to ensure that the header of subsequent pages is set correctly. 
	\item Using \detokenize{\left}, \detokenize{\right} in footnotes causes latexmk to loop infinitely. 
\end{itemize}




